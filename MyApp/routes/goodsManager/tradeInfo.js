...
router.get('/listView', function (req, res, next) {
    res.sendfile('views/goodsManager/new/tradeList.html');
});
router.get('/listView1', function (req, res, next) {
    res.sendfile('views/goodsManager/new/tradeList1.html');
});
...
router.get('/jinhuoList', function (req, res, next) {
    ...
    var sql = "select a.*,(a.number-ifnull(b.totalSellNumber,0)) nowNumber,0 as sellPrice,0 as sellNumber  " +
        "from t_purchase_record a LEFT JOIN (select purchaseId,sum(sellNumber) totalSellNumber from t_sell_record where goodsId=" + id + " GROUP BY purchaseId) b " +
...
router.get('/list-bak', function (req, res, next) {
    ...
    var sql = "select t.*,g.pinming,g.jinjia,g.guige from tradeRecord t left join goods g on g.id=t.shangpinId where 1=1 ";
    var params = [userId];
        ...
        query(sql1, params, function (qerr1, vals1, fields) {
            ...
            for (var item in vals) {
                ...
                if (vals[item]["type"] == "出货") {
                    vals[item]["maoli"] = ((vals[item]["sellPrice"] - vals[item]["purPrice"]) * vals[item]["number"]).toFixed(2);
                    vals[item]["maolilv"] = ((vals[item]["sellPrice"] - vals[item]["purPrice"]) / vals[item]["sellPrice"]).toFixed(2);
                } else {
                    vals[item]["maoli"] = 0;
                    vals[item]["maolilv"] = 0;
                }
...
router.get('/countTradeRecord', function (req, res, next) {
    ...
    var searchEndDate = req.param("searchEndDate");
    var param = [userId];
    var sql = "select type,sum(number*purPrice) as jinhuojine,sum(number*(purPrice)) as chengben,sum(number*sellPrice) as chushoujia from vw_trade_record_detail  " +
        "where belongUserId=? ";
...
router.post('/jinHuoSaveOrUpdate', function (req, res, next) {
    ...
    if (id == "" || id == undefined) {
        ...
        var userId = req.session.currentUser.id;
        sql = "insert into t_purchase_record(id,goodsId,number,price,date,remark,belongUserId) values(REPLACE(UUID(),'-',''),?,?,?,?,?,?)";
...
router.post('/chuHuoSaveOrUpdate', function (req, res, next) {
    ...
    if (id == "" || id == undefined) {
        ...
        sql = "insert into t_sell_record(id,goodsId,purchaseId,price,date,sellNumber,sellPrice,remark,belongUserId) values(REPLACE(UUID(),'-',''),?,?,?,?,?,?,?,?)";
...
router.get('/countSell', function (req, res, next) {
    ...
    var searchEndDate = req.param("searchEndDate");
    var param = [];
    var sql = "select sum(t.jiage*t.shuliang) jine,sum(g.jinjia*t.shuliang) chengben,type lirun from tradeRecord t left join goods g on g.id=t.shangpinId where 1=1";
    var orderSql = "order by type desc ";
    var groupBySql = " GROUP BY type "
    var whereSql = "";
